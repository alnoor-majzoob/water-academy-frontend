    ...
    <div className="p-6 space-y-6 overflow-y-auto h-full">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        <div className={`${card} lg:col-span-2`}>
          ...
          <ResponsiveContainer width="100%" height={200}><BarChart data={chartData} barSize={28}><CartesianGrid strokeDasharray="3 3" stroke={isDark ? '#334155' : '#f1f5f9'} /><XAxis dataKey="name" tick={{ fontSize: 11, fill: isDark ? '#94a3b8' : '#64748b' }} /><YAxis tick={{ fontSize: 11, fill: isDark ? '#94a3b8' : '#64748b' }} /><Tooltip formatter={(value: number) => [value, t('الدورات', 'Courses', lang)]} contentStyle={{ background: isDark ? '#1e293b' : '#fff', border: isDark ? '1px solid #334155' : '1px solid #e2e8f0', borderRadius: 12, fontSize: 12 }} labelStyle={{ color: isDark ? '#f1f5f9' : '#1e293b' }} /><Bar dataKey="courses" fill="#3B82F6" radius={[6, 6, 0, 0]} /></BarChart></ResponsiveContainer>
        </div>
        <div className={`${card}`}>
